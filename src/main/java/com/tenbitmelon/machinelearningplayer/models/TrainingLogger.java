...
public class TrainingLogger {
    ...
    public TrainingLogger(ExperimentConfig args, String logDir) {
        ...
        try {
            ...
            writer.write("timestamp,iteration,learning_rate,value_loss,policy_loss,entropy,old_approx_kl,approx_kl,clipfrac,explained_variance,iteration_time,SPS,num_terminations,num_truncations,average_rewards,total_rewards,bow_selected_steps,bow_drawing_steps,bow_fully_drawn_steps,shield_using_steps,gpu_mem_used,java_native_used\n");
            writer.flush();
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    public void logStep(long iteration, double learningRate, double valueLoss, double policyLoss, double entropy,
                        Double oldApproxKl, Double approxKl, double clipfrac, double explainedVariance, double iterationTime, double sps, int numTerminations, int numTruncations, double averageRewards, double totalRewards, int bowSelectedSteps, int bowDrawingSteps, int bowFullyDrawnSteps, int shieldUsingSteps, long gpuMemUsed, long javaNativeUsed) throws IOException {
        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        writer.write(String.format("%s,%d,%.6f,%.6f,%.6f,%.6f,%s,%s,%.6f,%.6f,%.6f,%.6f,%d,%d,%.6f,%.6f,%d,%d,%d,%d,%d,%d\n",
            ...
            bowSelectedSteps, bowDrawingSteps, bowFullyDrawnSteps, shieldUsingSteps, gpuMemUsed, javaNativeUsed));
        writer.flush();
    }
